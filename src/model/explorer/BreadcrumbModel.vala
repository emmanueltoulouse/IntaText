...
private void parse_path(string path) {
    ...
    if (path == "/" || path == "") {
        try {
            var icon = GLib.Icon.new_for_string("drive-harddisk");
            segments.add(new BreadcrumbSegment("/", "/", icon));
        } catch (Error e) {
            warning("Impossible de charger l'icône du disque: %s", e.message);
            segments.add(new BreadcrumbSegment("/", "/", null));
        }
        return;
    }
    ...
    try {
        var root_icon = GLib.Icon.new_for_string("drive-harddisk");
    ...
    if (path.has_prefix(home_path)) {
        ...
        try {
            var home_icon = GLib.Icon.new_for_string("user-home");
            segments.add(new BreadcrumbSegment("Accueil", home_path, home_icon));
        } catch (Error e) {
            warning("Impossible de charger l'icône du home: %s", e.message);
            segments.add(new BreadcrumbSegment("Accueil", home_path, null));
        }
    ...
    try {
        foreach (string part in parts) {
            ...
            if (!segment_name.validate()) {
                warning("BreadcrumbModel: Nom de segment non UTF-8 détecté: '%s' (partie de '%s')", segment_name,
                    original_path);
            }
            if (segment_path == null || segment_path == "") {
                warning(
                    "BreadcrumbModel: Chemin absolu calculé vide pour segment '%s' (partie de '%s'). 'current' était '%s'. Original absolu: %s",
            ...
            try {
                var folder_icon = GLib.Icon.new_for_string("folder");
                segments.add(new BreadcrumbSegment(segment_name, segment_path, folder_icon));
            } catch (Error e) {
                warning("Impossible de charger l'icône du dossier: %s", e.message);
                segments.add(new BreadcrumbSegment(segment_name, segment_path, null));
            }
        }
    } catch (Error e) {
        warning("Erreur lors du parsing du chemin: %s", e.message);
    }
}
