...
/**
  ...
  */
private void search_files_internal(string path, string pattern, bool recursive,
    Gee.ArrayList<FileItemModel> results) throws Error {
    var directory = File.new_for_path(path);
    var enumerator = directory.enumerate_children(
        "standard::*,time::modified,unix::mode",
        FileQueryInfoFlags.NONE
...
/**
  ...
  */
public Gee.ArrayList<FileItemModel> search_in_file_contents(string path, string text, int max_results = 100,
    bool recursive = true) {
    var results = new Gee.ArrayList<FileItemModel>();
...
/**
  ...
  */
private void search_content_internal(string path, string text, int max_results, bool recursive,
    Gee.ArrayList<FileItemModel> results) throws Error {
...
/**
  ...
  */
private void search_by_time_internal(string path, DateTime cutoff_time, bool recursive,
    Gee.ArrayList<FileItemModel> results) throws Error {
    var directory = File.new_for_path(path);
    var enumerator = directory.enumerate_children(
        "standard::*,time::modified,unix::mode",
...
public Gee.List<string> get_available_extensions_from_ini() {
    var config_dir = Environment.get_user_config_dir() + "/intatext";
