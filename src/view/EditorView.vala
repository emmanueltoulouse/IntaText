...
namespace IntaText {
    public class EditorView : Gtk.Box {
        ...
        public EditorView(ApplicationController controller) {
            ...
            // Emballer ce groupe dans un FlowBoxChild
            var text_group_container = new FlowBoxChild();
            text_group_container.set_child(text_group);
            toolbar_box.append(text_group_container);
            ...
            // Emballer ce groupe dans un FlowBoxChild
            var para_group_container = new FlowBoxChild();
            para_group_container.set_child(para_group);
            toolbar_box.append(para_group_container);
            ...
            // Emballer ce groupe dans un FlowBoxChild
            var special_group_container = new FlowBoxChild();
            special_group_container.set_child(special_group);
            toolbar_box.append(special_group_container);

            // Créer l'éditeur WYSIWYG
            wysiwyg_editor = new WysiwygEditor();
            wysiwyg_editor.set_vexpand(true);
            wysiwyg_editor.set_hexpand(true);
            content_box.append(wysiwyg_editor);
            ...
            var font_dropdown = new Gtk.DropDown.from_strings(fonts);
            font_dropdown.set_selected(find_index_in_array(fonts, font_family));
            style_group.append(font_dropdown);

            // --- Taille (Gtk.DropDown) ---
            var size_dropdown = new Gtk.DropDown.from_strings(sizes);
            ...
            var style_group_container = new FlowBoxChild();
            style_group_container.set_child(style_group);
            toolbar_box.append(style_group_container);
            ...
            wysiwyg_editor.get_buffer().notify["cursor-position"].connect(() => {
